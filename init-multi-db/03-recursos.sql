...
CREATE TABLE recursos_fisicos (
    ...
    tipo_recurso       VARCHAR(50)  NOT NULL CHECK (
        tipo_recurso IN ('Libro','Notebook','Tablet','Juego de mesa')
    ),
    isbn               VARCHAR(20),
    estado_fisico      VARCHAR(50)  DEFAULT 'Excelente' CHECK (
        estado_fisico IN ('Excelente','Buen estado','Dañado','En reparación')
    ),
    disponible         BOOLEAN      DEFAULT TRUE,

    CONSTRAINT fk_recursos_isbn
        FOREIGN KEY (isbn)
        REFERENCES libros_proyeccion(isbn),

    CONSTRAINT chk_isbn_segun_tipo
        CHECK (
            (tipo_recurso = 'Libro' AND isbn IS NOT NULL)
            OR
            (tipo_recurso <> 'Libro')
        )
) ENGINE=InnoDB;
...
CREATE TABLE historial_eventos_recursos_fisicos (
    ...
    CONSTRAINT fk_historial_recurso
        FOREIGN KEY (recurso_id)
        REFERENCES recursos_fisicos(id)
) ENGINE=InnoDB;
...
CREATE TABLE mantenimiento_recursos (
    ...
    estado             VARCHAR(40)  NOT NULL CHECK (
        estado IN ('Abierto','Cerrado','Cancelado')
    ),
...
CREATE INDEX idx_recursos_sku
    ON recursos_fisicos(sku);

CREATE INDEX idx_recursos_disponible
    ON recursos_fisicos(disponible);

CREATE INDEX idx_historial_recurso
    ON historial_eventos_recursos_fisicos(recurso_id);

CREATE INDEX idx_historial_usuario
    ON historial_eventos_recursos_fisicos(usuario_email);

-- DATOS DE PROYECCIÓN
INSERT INTO libros_proyeccion (isbn, titulo) VALUES
('9798344055985', 'Moby Dick'),
('9788437604947', 'Cien años de soledad'),
('9788420651323', 'El Principito'),
('9780141036137', '1984'),
('9788497592208', 'El resplandor'),
('9781537822075', 'Dracula'),
('9788420674209', 'Don Quijote'),
('9781644732076', 'Harry Potter'),
...
(sku, tipo_recurso, isbn, estado_fisico, disponible) VALUES
...
(usuario_email, recurso_id, fecha_evento, estado) VALUES
...
INSERT INTO mantenimiento_recursos
(recurso_id, fecha_inicio, estado, observacion) VALUES
